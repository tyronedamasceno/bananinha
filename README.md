...
```bash
dk bash  # starts bash inside "bananinha" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and bananinha.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `bananinha` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The bananinha container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv bananinha -p python3  # creates a python3 virtualenv
...
Install docker and nginx. Create an empty postgres database bananinha owned by a user bananinha.

On your remote machine, create a file ~/bananinha.env:

```
DJANGO_DB_PASSWORD=<bananinha's password>
DJANGO_DB_HOST=<database_ip>
DJANGO_DB_NAME=bananinha
DJANGO_DB_USER=bananinha
DJANGO_DEBUG=0
```
...
```
server {
    server_name  bananinha.example.com;
    ...
    location /static {
        alias /home/ubuntu/dkdata/bananinha/static;
...
(Replace "bananinha.example.com" with your production domain)
