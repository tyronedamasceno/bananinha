...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=bananinha.example.com
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development bananinha npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop bananinha
    docker rm bananinha
    docker run --name bananinha -d --env-file /home/ubuntu/bananinha.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/bananinha:/dkdata \
        bananinha start_web.sh
}
...
echo_green "Welcome to bananinha's dev env"
